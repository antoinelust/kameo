...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 if($user != '' && $ID!='') {
    ...
    $sql = "UPDATE conditions SET HEU_MAJ=CURRENT_TIMESTAMP, USR_MAJ='$user', BOOKING='$booking', ASSISTANCE='$assistance', LOCKING='$locking' WHERE COMPANY='$internalReference'";
    if ($conn->query($sql) === FALSE) {
        $response = array ('response'=>'error', 'message'=> $conn->error);
        echo json_encode($response);
        die;
