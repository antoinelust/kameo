...
$dateStart2= clone $dateStart;
$dateStart2->sub(new DateInterval("PT45M"));
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' && $intake_building != NULL & $dateStart != NULL && $deposit_building != NULL && $dateEnd != NULL ) {
    ...
    while($row = mysqli_fetch_array($result))
    {
        ...
        $resultKeyNotReturned = mysqli_query($conn, $sqlKeyNotReturned);
        $lengthKeyNotReturned = $resultKeyNotReturned->num_rows;
        if($lengthKeyNotReturned == 0 || $dateStart2 >= (new DateTime('NOW', new DateTimeZone('Europe/Brussels')))){
