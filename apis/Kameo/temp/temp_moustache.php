...
$sql="SELECT * from bike_catalog WHERE SEASON='2020-2021' AND BRAND='Moustache Bikes'";
...
while($row = mysqli_fetch_array($result)){
  ...
  $fichier =  $dossier.strtolower(str_replace(" ", "-", "moustache"))."_".strtolower(str_replace(".", "_", str_replace(" ", "_", $row["MODEL"])))."_".strtolower($row["FRAME_TYPE"]).".jpg";
  $fichier_mini =  $dossier.strtolower(str_replace(" ", "-", "moustache"))."_".strtolower(str_replace(".", "_", str_replace(" ", "_", $row["MODEL"])))."_".strtolower($row["FRAME_TYPE"])."_mini.jpg";
...
while($row = mysqli_fetch_array($result)){
  ...
  if (!file_exists($fichier)){
    echo "fichier manquant ! : "."moustache"." ".$row['MODEL']." ".$ROW['FRAME_TYPE']." - ".$fichier."\n";
  }else{
    echo "fichier présent !\n";
    echo "taille de l'image : ".(getimagesize($fichier)[0]/getimagesize($fichier)[1])."\n";
  }
  if (!file_exists($fichier_mini)){
    echo "fichier mini manquant ! : "."moustache"." ".$row['MODEL']." ".$ROW['FRAME_TYPE']." - ".$fichier_mini."\n";
  }else{
