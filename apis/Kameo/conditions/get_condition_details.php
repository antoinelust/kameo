...
if ($stmt)
{
  ...
  if($response['NAME']=="generic"){
    ...
    $stmt = $conn->prepare("select aa.EMAIL, bb.NOM, bb.PRENOM from specific_conditions aa, customer_referential bb where CONDITION_REFERENCE=? AND aa.EMAIL=bb.EMAIL");
    if ($stmt)
    {
      $stmt->bind_param("i", $ID);
      $stmt->execute();
