...
while($row = mysqli_fetch_array($result))
{
    ...
    $sql_dateStart="SELECT * from ((select min(SUBSTR(CONTRACT_START, 9, 2)) as 'start', COMPANY, BILLING_GROUP from customer_bikes aa where aa.CONTRACT_START<='$dateStart' and (aa.CONTRACT_END>'$dateStart' or aa.CONTRACT_END is NULL) and aa.COMPANY='$internalReference' and aa.BILLING_GROUP='$billingGroup') UNION (select min(SUBSTR(START, 9, 2)) as 'start', COMPANY, BILLING_GROUP from boxes aa where aa.START<='$dateStart' and (aa.END>'$dateStart' or aa.END is NULL) and aa.COMPANY='$internalReference' and aa.BILLING_GROUP='$billingGroup') UNION (SELECT min(SUBSTR(accessories_stock.CONTRACT_START, 9, 2)), companies.INTERNAL_REFERENCE as '$internalReference', accessories_stock.BILLING_GROUP FROM accessories_stock, companies WHERE companies.ID=accessories_stock.COMPANY_ID AND accessories_stock.CONTRACT_TYPE='leasing' AND accessories_stock.CONTRACT_START<'$dateStart' AND accessories_stock.CONTRACT_END>'$dateStart' AND accessories_stock.STAANN != 'D' AND companies.INTERNAL_REFERENCE='$internalReference')) AS T1 WHERE start is NOT NULL";
