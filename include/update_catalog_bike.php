...
if($action=="update"){
    ...
    if(isset($_FILES['widget-updateCatalogMini-form-file'])){

        $extensions = array('.jpg');
        $extension = strrchr($_FILES['widget-updateCatalogMini-form-file']['name'], '.');
        if(!in_array($extension, $extensions))
        {
              errorMessage("ES0041");
        }
        ...
        $taille_maxi = 6291456;
        $taille = filesize($_FILES['widget-updateCatalogMini-form-file']['tmp_name']);
        if($taille>$taille_maxi)
        {
              errorMessage("ES0023");
        }
        ...
        $fichier = strtolower(str_replace(" ", "-", $brand))."_".strtolower(str_replace(" ", "-", $model))."_".strtolower($frameType)."_mini".$extension;
         ...
         if(move_uploaded_file($_FILES['widget-updateCatalogMini-form-file']['tmp_name'], $dossier . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
         {
            $upload=true;
            $path= $dossier . $fichier;
         }
