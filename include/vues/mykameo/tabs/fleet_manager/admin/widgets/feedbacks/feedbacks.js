  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.response == "success") {
        document.getElementById("counterFeedbacks").innerHTML = '<span style="margin-left:20px">'+response.feedbacksNumber+'</span>';
      }
    },
  });
    ...
    success: function (response) {
      ...
      if (response.response == "success") {
        ...
        while (i < response.feedbacksNumber) {
          ...
          var temp =
            '<tr><td><a href="#" class="text-green retrieveFeedback" data-target="#feedbackManagement" name="' +
            response.feedback[i].IDReservation +
            '" data-toggle="modal">' +
            response.feedback[i].IDReservation +
            "</a></td><td>" +
            ...
            "</td><td>" +
            response.feedback[i].end.shortDateHours() +
            "</td><td>" +
            note +
            "</td><td>" +
            comment +
            ...
            entretien +
            "</td><td>" +
            response.feedback[i].firstName +
            " " +
            response.feedback[i].name +
            "</td><td>" +
            response.feedback[i].status +
            "</td><td>" +
            response.feedback[i].read +
            "</tr>";
          dest = dest.concat(temp);
          i++;
        }
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.response == "success") {
        ...
        $("#feedbackManagement input[name=startDate]").val(
          response.start.shortDateHours()
        );
        $("#feedbackManagement input[name=endDate]").val(
          response.end.shortDateHours()
        );
