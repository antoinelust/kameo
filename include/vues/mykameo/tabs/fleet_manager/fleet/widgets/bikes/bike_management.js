...
function fillBikeDetails(element)
{
    ...
    $.ajax({
            ...
            success: function(response){
                if (response.response == 'error') {
                    ...
                    document.getElementsByClassName("bikeImage")[0].src="images_bikes/"+response.img+"_mini.jpg";
    ...
    $.ajax({
            url: 'apis/Kameo/action_bike_management.php',
            type: 'post',
            data: { "readActionBike-action": "read", "readActionBike-bikeNumber": bikeID, "readActionBike-user": "<?php echo $user_data['EMAIL']; ?>"},
            success: function(response){
                if (response.response == 'error') {
                    console.log(response.message);
                } else{
