...
CREATE TABLE `kameobiknqdataba`.`companies_contact` ( `ID` INT(3) NOT NULL AUTO_INCREMENT , `NOM` VARCHAR(250) NOT NULL , `PRENOM` VARCHAR(250) NOT NULL , `EMAIL` VARCHAR(250) NOT NULL , `PHONE` VARCHAR(15) NULL , `ID_COMPANY` INT NOT NULL,`FUNCTION` VARCHAR(250) NULL , `BIKES_STATS` VARCHAR(1) NOT NULL, PRIMARY KEY (`ID`)) ENGINE = MyISAM;
...
/*Ajout de champs*/
ALTER TABLE `notifications` ADD `TYPE_ITEM` INT NULL AFTER `USER_ID`, ADD `STAAN` VARCHAR(1) NULL AFTER `TYPE_ITEM`;
ALTER TABLE `notifications` CHANGE `HEU_MAJ` `HEU_MAJ` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP;


/*=====COMPANIES_CONTACT=====*/
/*Ajout des champs HEU_MAJ et USR_MAJ*/
ALTER TABLE `companies_contact` ADD `HEU_MAJ` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `ID`, ADD `USR_MAJ` VARCHAR(155) NOT NULL AFTER `HEU_MAJ`;


/*=====COMPANY_ACTIONS=====*/
ALTER TABLE `company_actions` ADD `CHANNEL` VARCHAR(150) NULL DEFAULT NULL AFTER `OWNER`;

/*=====ENTRETIENS=====*/
/*Création de la table*/
CREATE TABLE `kameobiknq`.`entretiens` ( `ID` INT(10) NOT NULL AUTO_INCREMENT, `HEU_MAJ` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `USR_MAJ` VARCHAR(150) NOT NULL , `BIKE_ID` INT(10) NOT NULL , `DATE` TIMESTAMP NOT NULL , `STATUS` VARCHAR(100) NOT NULL , `COMMENT` TEXT NULL , PRIMARY KEY (`ID`)) ENGINE = MyISAM;
/*ajout de la clé étrangère*/
ALTER TABLE entretiens
ADD FOREIGN KEY (BIKE_ID) REFERENCES customer_bikes(ID);
