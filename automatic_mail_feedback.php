...
$partie2="   <body>
        <!--*|IF:MC_PREVIEW_TEXT|*-->
        <!--[if !gte mso 9]><!----><span class=\"mcnPreviewText\" style=\"display:none; font-size:0px; line-height:0px; max-height:0px; max-width:0px; opacity:0; overflow:hidden; visibility:hidden; mso-hide:all;\">Votre avis nous intéresse ! </span><!--<![endif]-->
        <!--*|END:IF|*-->
        <center>
            ...
            <td valign=\"top\" class=\"mcnTextBlockInner\" style=\"padding-top:9px;\">
                ...
                <table align=\"left\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:100%; min-width:100%;\" width=\"100%\" class=\"mcnTextContentContainer\">
                    <tbody><tr>
                        ...
                        <br><br>
                            <h3>Votre santé et l'environnement vous remercient de vous être déplacé à vélo!</h3>

                            <p>Vous venez d'utiliser un vélo KAMEO Bikes et votre avis nous intéresse.</p>
                            <p>Donnez-nous votre <strong>feedback</strong> pour que l'utilisateur suivant profite pleinement du vélo.</p>
                            <br>
                            <p>D'avance merci,<br>
                            L'équipe <strong>KAMEO Bikes</strong></p>
                        </td>
                    </tr>
...
while($row = mysqli_fetch_array($result))
{
    ...
    echo "Bike Number: ".$frame_number."<br/>";
    echo "Start of booking: ".date("Y-m-d H:i:s", $dateStart)."<br/>";
    echo "End of booking: ".date("Y-m-d H:i:s", $dateEnd)."<br/>";
    
    include 'include/connexion.php';
    $email2='antoine.lust@kameobikes.com';
    
    $sql2="SELECT * FROM  customer_referential where EMAIL='$email'";
    ...
    $name=$resultat2['NOM'];
    $firstName=$resultat2['PRENOM'];

    echo "CONTACT: ".$email." - ". $firstName ." ". $name ."<br/>";
    
    
    $partie2=$partie2."<td align=\"center\" valign=\"middle\" class=\"mcnButtonContent\" style=\"font-family: Helvetica; font-size: 18px; padding: 18px;\">
    <a class=\"mcnButton \" title=\"Donner mon feedback sur la réservation".$id." \" href=\"www.kameobikes.com/mykameo.php?feedback=".$id."\" target=\"_blank\" style=\"font-weight: bold;letter-spacing: -0.5px;line-height: 100%;text-align: center;text-decoration: none;color: #FFFFFF;\">Donner mon feedback sur la réservation ".$id."</a>";

    include 'include/connexion.php';
    $sql2="INSERT INTO feedbacks (HEU_MAJ, USR_MAJ, BIKE_NUMBER, ID_RESERVATION, NOTE, COMMENT, ENTRETIEN, STATUS) VALUES (CURRENT_TIMESTAMP, 'mykameo', '$frame_number', '$id', NULL, NULL, NULL, 'SENT')";
    if ($conn->query($sql2) === FALSE) {
        $response = array ('response'=>'error', 'message'=> $conn->error);
        echo json_encode($response);
        die;
    ...
    if(substr($_SERVER['REQUEST_URI'], 1, 4) != "test" && substr($_SERVER['HTTP_HOST'], 0, 9)!="localhost"){
        $mail->AddAddress($email2, $firstName.' '.$name);
    }else{
    ...
    $mail->Subject = "Fin de réservation - Feedback";


    $mail->Body = $part1.$partie2;
    if(substr($_SERVER['HTTP_HOST'], 0, 9)!="localhost"){
        if(!$mail->Send()) {
           ...
           echo 'Mail envoyé à : antoine<br/>';
        }    
    }else{
        echo 'Environnement localhost, mail non-envoyé<br/>';
    }
